...
try:
    ...
    subprocess.run(["pg_dump", "--schema-only", "--no-owner", source_url, "-f", temp_schema], check=True)
    subprocess.run(["psql", target_url, "-c", "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"], check=True)
    ...
    tables = subprocess.run(["psql", source_url, "-t", "-c", tables_query], 
                           capture_output=True, text=True, check=True).stdout.splitlines()
    
    # 3. Import data with dependency handling
    for table in [t.strip() for t in tables if t.strip()]:
        subprocess.run(["pg_dump", "--data-only", "--disable-triggers", "--table", f"public.{table}", 
                        "--rows=100", source_url], stdout=subprocess.PIPE, check=True).stdout | \
        subprocess.run(["psql", target_url], stdin=subprocess.PIPE, check=False)
